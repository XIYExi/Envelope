...
const getLayersForElement = (element) => {
  ...
  } else if (element instanceof SVGElement) return [];
  ...
  /*
     ...
     */
  if (
    (element.parentElement instanceof HTMLPictureElement &&
  ...
  if (
    ...
    if (rect.width >= 1 && rect.height >= 1) {
      ...
      if (strokes) Object.assign(rectNode, strokes);
      if (!rectNode.strokes) {
...
export function htmlToMastergo(selector, useFrames, time) {
  ...
  if (element) {
    ...
    for (const node of textNodes) {
      ...
      if (textNode) layers.push(textNode);
    }
  }
