...
// 粘贴
export function onPaste(graph: any) {
  const cells = graph?.getSelectedCells();
...
export function deleteCells(graph: any) {
  const cells = graph.getSelectedCells();
  if (cells.length) {
    // 删除前移除所有包含工具
    cells.forEach((currentCell: any) => {
...
// 全选
export function selectAll(graph: any) {
  const cells = graph.getCells();
  if (cells.length) {
    graph.select(cells);
  }
 ...
 * 冻结画布
 */
export function freezeGraph(graph: any) {
  const cells = graph.getCells();
  if (cells.length) {
 ...
 * 解冻
 */
export function unfreezeGraph(graph: any) {
  const cells = graph.getCells();
  if (cells.length) {
    cells.forEach((cell: any) => {
