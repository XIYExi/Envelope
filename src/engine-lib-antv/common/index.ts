...
/**文字溢出格式化 */
export function fmtLabelOverflow(label: any) {
...
/**监听单元事件双击回调 */
export function nodeDclick(cb: any) {
  Channel.eventListener(CustomEventTypeEnum.DOUBLE_NODE_CLICK, (detail: any) =>
    cb(detail),
  );
}

/**监听单元事件单击回调 */
export function nodeClick(cb: any) {
...
/**运行时异常回调 */
export function runtimeError(cb: any) {
  Channel.eventListener(CustomEventTypeEnum.RUNTIME_ERR, (err: any) => cb(err));
}
