...
/**获取不同actionType对应主题色 */
export function getActionTypeTheme(type: any) {
  /**@enum */
  const Theme = {
    /**默认深蓝 */
 ...
 * 兼容x6/g6
 */
function getBaseConfig(node: any) {
  let {
    type,
    shape,
    tooltip,
 ...
 * 获取多边形node节点
 */
export function getEllipseNode(node: any) {
  const { type, label, x, y, width, height, id, actionType, data } =
    getBaseConfig(node);
  // 主题色
 ...
 * 获取矩形node节点
 */
export function getRectNode(node: any) {
  const { type, label, x, y, width, height, id, actionType, data } =
    getBaseConfig(node);
  // 主题色
  const targetTheme = getActionTypeTheme(actionType);
 ...
 * 获取圆角矩形，表示可选操作
 */
export function getBorderedRectNode(node: any) {
  const { type, label, x, y, width, height, id, actionType, data } =
    getBaseConfig(node);
  // 主题色
  const targetTheme = getActionTypeTheme(actionType);
 ...
 * 获取平行四边形，用于表示数据
 */
export function getDataRectNode(node: any) {
  const { type, label, x, y, width, height, id, actionType, data } =
    getBaseConfig(node);
  // 主题色
  const targetTheme = getActionTypeTheme(actionType);
  return {
    id,
    inherit: type, // 指定使用何种图形，默认值为 'rect'
    width,
    height,
    x,
    y,
 ...
 * 获取圆形，用于表示连接
 */
export function getConnectCircleNode(node: any) {
  const { type, label, x, y, width, height, id, actionType, data } =
    getBaseConfig(node);
  // 主题色
  const targetTheme = getActionTypeTheme(actionType);
 ...
 * 获取菱形node节点
 */
export function getDiamondNode(node: any) {
...
export function getDagNode(node: any) {
...
export function getPRONode(node: any) {
  const { label, width, height, id, data } = getBaseConfig(node);
  return {
    id,
    shape: 'dag-node',
...
export function getALGONode(node: any) {
  const { label, width, height, id, data } = getBaseConfig(node);
  return {
    id,
    shape: 'dag-node',
...
export function getERNode(node: any) {
  const { label, width, height, id, data } = getBaseConfig(node);
  return {
    id,
    shape: 'er-rect',
...
// 获取Vue节点
export function getVueNode(node: any) {
  const { label, width, height, id, data } = getBaseConfig(node);
  return {
    id,
    shape: 'vue-shape',
...
function getNodeJSON(nodes: any) {
  const nodeList = [];
  for (const node of nodes) {
    ...
    if (!actionType) {
      Channel.dispatchEvent(
        ...
        ErrorClass.InvalidFormatData('缺少ActionType'),
      );
      // @ts-ignore
 ...
 * 按照指定的 JSON 数据渲染节点和边。
 */
export function fromJSON(graph: any, nodes: any, edges: any) {
  if (!Lang.isArray(nodes) || !Lang.isArray(edges)) {
    Channel.dispatchEvent(
      CustomEventTypeEnum.RUNTIME_ERR,
      ErrorClass.InvalidParameters('节点或者边数据格式不正确'),
    );
    // @ts-ignore
 ...
 * 序列化
 */
export function toJSON(graph: any) {
  const edges = [],
    nodes = [],
    edgesJSON = [],
    nodesJSON = [];
