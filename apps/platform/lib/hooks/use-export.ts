/**
 * 标准项目导出 WebSocket 管理 Hook
 *
 * 封装导出任务的创建、WebSocket 监听与下载流程。
 *
 * @author xiye
 * @date 2026-06-25
 */
...
export function useExport({ projectId, dirty, flushAutosave }: UseExportOptions) {
  ...
  const webSocket = useTaskWebSocket({
    ...
    onDone() {
      ...
      if (url && projectId) {
        downloadFile(url, `project-${projectId.slice(0, 8)}.zip`).catch((e) => {
  ...
  return { isExporting, error, stage, percent, startExport };
}
